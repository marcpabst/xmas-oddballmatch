...
def perform_ica(id):
    ...
    # Cut non-block data
    raw = utils.cut_raw_blocks(raw, events, config["events_dict"], "begin", "end")

    # Remove line noise using ZapLine
    raw = utils.apply_zapline(raw, config["line_freq"], picks = ["eeg"], nremove = 2)
    ...
    ica = mne.preprocessing.ICA(n_components=n_pca_components, method="picard", fit_params=None, random_state=config["random_state"])

    # Fit ICA
    ica.fit(epochs, picks=["eeg"], reject = {"eeg": config["ica_diff_criterion"]})
...
def main():
    ...
    if args.subjects:
        Parallel(n_jobs=config["njobs"], prefer="threads")(delayed(perform_ica)(id) for id in args.subjects)
    else:
        ids = get_entity_vals(config["bids_root_path"], "sub")
        Parallel(n_jobs=config["njobs"], prefer="threads")(delayed(perform_ica)(id) for id in ids)
