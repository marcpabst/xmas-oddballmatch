...
)
{
  ...
  args <- env %>%
    rstatix:::.add_item(method = "t_test")
  params <- env %>%
    rstatix:::remove_null_items() %>%
    rstatix:::add_item(method = "ttestBF_wrapper")
  ...
  if(number.of.groups > 2 & !is.null(ref.group)){
    if(ref.group %in% c("all", ".all.")){
      params$data <- rstatix:::create_data_with_all_ref_group(data, outcome, group)
      params$ref.group <- "all"
    }
  }
  test.func <- rstatix:::two_sample_test
  if(number.of.groups > 2)
    test.func <- rstatix:::pairwise_two_sample_test
  do.call(test.func, params) %>%
    rstatix:::set_attrs(args = args) %>%
    rstatix:::add_class(c("rstatix_test", "t_test"))
}
